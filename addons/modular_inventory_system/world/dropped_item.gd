...
func _ready() -> void:
	...
	add_to_group("dropped_item") # <--- ADICIONE ESTA LINHA!
...
func _setup_model() -> void:
	...
	# 2. Se não tem modelo 3D, mas tem ícone 2D (A sua Maçã vai cair aqui!)
		...
		# Faz a maçã estar sempre "de frente" para a câmera do jogador, não importa o ângulo
		sprite.billboard = BaseMaterial3D.BILLBOARD_ENABLED
		
		# Define o tamanho do sprite no mundo 3D (ajuste esse valor se ficar muito grande ou pequeno)
